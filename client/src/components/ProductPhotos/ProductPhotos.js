...
export default function ProductPhotos() {
    ...
    const getProductPhotos = async (id) => {
        try {
            const response = await axios.get(`https://hajnalszabolcs.duckdns.org:5000/api/images/${id}`);
            console.log("response1: ", response.data);
            const photoURLs = response.data.map(photo => ({
                id: photo.id,
                url: `https://hajnalszabolcs.duckdns.org:5000/${photo.url.replace(/\\/g, '/')}`
            }));
            setPhotos(photoURLs);
        } catch (error) {
    ...
    const deletePhoto = async (photoId) => {
        try {
            await axios.delete(`https://hajnalszabolcs.duckdns.org:5000/api/delete_image/${photoId}`);
