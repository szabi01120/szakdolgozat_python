...
export default function Products() {
  ...
  useEffect(() => {
    axios
      .get('https://dezsanyilvantarto.hu:5000/api/products')
      .then((response) => {
  ...
  const handleConfirmDelete = async () => {
    try {
      const response = await axios.delete(`https://dezsanyilvantarto.hu:5000/api/delete_product/${productIdToDelete}`);
  ...
  const handleCheckboxChange = async (index, field) => {
    ...
    try {
      const response = await axios.put(`https://dezsanyilvantarto.hu:5000/api/update_checkbox_state/${product.id}`, {
        [field]: product[field],
      });
  ...
  const handleSaveEdit = async () => {
    try {
      const response = await axios.put(`https://dezsanyilvantarto.hu:5000/api/update_product/${editingProductId}`, editedProduct, { withCredentials: true });
      if (response.status === 200) {
  ...
  const handleSaveButtonClick = async () => {
    if (productsToMove.length > 0) {
      try {
        ...
        const response = await axios.post('https://dezsanyilvantarto.hu:5000/api/update_product_status', productsToMoveDetails);
